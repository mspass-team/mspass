...
class DesktopLauncher(BasicMsPASSLauncher):
    ...
    already fully running is treated as caller-owned.  Browser processes
    started by this object are always reaped.  Compose execution failures are
    reported as ``RuntimeError``; invalid constructor arguments are reported
    as ``ValueError``.
    """
    ...
    def launch(self):
        ...
        startup fails.  A fully running project discovered on entry is not
        brought down by this object.
