...
if [ "$1" != "--batch" ]; then
  ...
  # echo "Notebook Token:" $MSPASS_JUPYTER_PWD
fi 
...
if [ $# -eq 0 ] || [ $1 = "--batch" ]; then
  ...
  function start_mspass_frontend {
    ...
    MSPASS_JUPYTER_PWD_HASHED=$(python3 -c "from notebook.auth import passwd; print(passwd('${MSPASS_JUPYTER_PWD}'))") > nb_stdout.txt 2>&1
    NOTEBOOK_ARGS="${NOTEBOOK_ARGS} --NotebookApp.token=${MSPASS_JUPYTER_PWD}"

    if [ "$MSPASS_SCHEDULER" = "spark" ]; then
      export PYSPARK_DRIVER_PYTHON=jupyter
      if [ -z $1 ]; then
        export PYSPARK_DRIVER_PYTHON_OPTS="lab ${NOTEBOOK_ARGS}" > nb_stdout.txt 2>&1  
      else
        export PYSPARK_DRIVER_PYTHON_OPTS="nbconvert ${BATCH_MODE_ARGS}" > nb_stdout.txt 2>&1
      fi
      pyspark \
        ...
        --packages org.mongodb.spark:mongo-spark-connector_2.12:3.0.0 > nb_stdout.txt 2>&1
    else # if [ "$MSPASS_SCHEDULER" = "dask" ]
      export DASK_SCHEDULER_ADDRESS=${MSPASS_SCHEDULER_ADDRESS}:${DASK_SCHEDULER_PORT}
      if [ -z $1 ]; then
        jupyter lab ${NOTEBOOK_ARGS} > nb_stdout.txt 2>&1
      else
        jupyter nbconvert ${BATCH_MODE_ARGS}> nb_stdout.txt 2>&1
      fi
    fi
  }
