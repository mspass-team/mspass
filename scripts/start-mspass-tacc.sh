...
if [ "$1" != "--batch" ]; then
  ...
  echo "http://$TAPIS_EXEC_SYSTEM.tacc.utexas.edu:$LOGIN_PORT/lab?token=$MSPASS_JUPYTER_PWD"| sed  -e :a -e "s/^.\{1,$(tput cols)\}$/ & /;ta" | tr -d '\n' | head -c $(tput cols);echo -e "\n"
  printf '%*s\n' "${COLUMNS:-$(tput cols)}" '' | tr ' ' -
