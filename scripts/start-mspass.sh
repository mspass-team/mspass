...
if [ $# -eq 0 ] || [ $1 = "--batch" ]; then
  ...
  if [ "$MSPASS_SCHEDULER" = "spark" ]; then
    ...
    MSPASS_WORKER_CMD='dask-worker ${MSPASS_WORKER_ARG} --memory-limit=0 --local-directory $MSPASS_WORKER_DIR tcp://$MSPASS_SCHEDULER_ADDRESS:$DASK_SCHEDULER_PORT > ${MSPASS_LOG_DIR}/dask-worker_log_${MY_ID} 2>&1 &'
  fi
