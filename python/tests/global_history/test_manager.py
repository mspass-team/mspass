...
class TestManager:
    ...
    def test_mspass_map(self, spark_context):
        ...
        assert (
            docs[0]["parameters"]
            == docs[1]["parameters"]
            == "bandpass,freqmin=1,freqmax=5,object_history=True"
        )
    ...
    def test_mspass_reduce(self, spark_context):
        ...
        assert (
            manager_db["history_global"].count_documents(
                {"alg_name": "stack", "parameters": "object_history=True,alg_id=3"}
            )
            == 2
        )
        ...
        assert (
            manager_db["history_global"].count_documents(
                {"alg_name": "new_stack", "parameters": "object_history=True,alg_id=3"}
            )
            == 1
        )
    ...
    def test_get_alg_id(self):
        ...
        assert (
            self.manager.get_alg_id("new_stack", "object_history=True,alg_id=3")
            == res["alg_id"]
        )
