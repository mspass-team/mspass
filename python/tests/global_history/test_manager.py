...
def dask_map(input, manager, alg_name=None, parameters=None):
    ...
    res = ddb.mspass_map(signals.filter, "bandpass", freqmin=1, freqmax=5, object_history=True, global_history=manager, alg_name=alg_name, parameters=parameters)
    return res.compute()
...
class TestManager():
    ...
    def test_mspass_map(self):
        ...
        assert manager_db['history_global'].count_documents({'job_name': self.manager.job_name}) == 2
        assert manager_db['history_global'].count_documents({'alg_id': spark_alg_id}) == 2
        ...
        dask_res = dask_map(l, self.manager)
        assert manager_db['history_global'].count_documents({'job_name': self.manager.job_name}) == 3
        assert manager_db['history_global'].count_documents({'alg_id': spark_alg_id}) == 3
        ...
        assert manager_db['history_global'].count_documents({'alg_id': spark_alg_id}) == 5
        ...
        assert manager_db['history_global'].count_documents({'alg_name': 'new_filter_2'}) == 1
    ...
    def test_mspass_reduce(self):
        ...
        assert manager_db['history_global'].count_documents({'alg_id': spark_alg_id}) == 1
        ...
        assert manager_db['history_global'].count_documents({'alg_id': dask_alg_id}) == 2
