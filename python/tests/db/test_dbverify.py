...
class TestDBVerify():
    ...
    def test_main(self, capfd):
        ...
        save_res_code = self.db.save_data(ts1, mode='promiscuous', storage_mode='gridfs')
        save_res_code = self.db.save_data(ts2, mode='promiscuous', storage_mode='gridfs')
        # erase required attributes
        save_res_code = self.db.save_data(ts3, mode='promiscuous', storage_mode='gridfs', exclude_keys=['starttime'])
        ...
        dbverify.main(['test_dbverify', '-t', 'normalization', '-c', 'wf_TimeSeries', 'site'])
        out, err = capfd.readouterr()
