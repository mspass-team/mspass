...
class TestNormalize():
    ...
    def test_mseed_channel_matcher_get_document(self):
        ...
        uncached_matcher = mseed_channel_matcher(self.db, cache_normalization_data=False)
        ...
        assert(ts_1.dead())
        assert(ts_2.dead())
        assert("No match for net_sta_chan_loc =TA_034A_BHE and time=" in str(ts_1.elog.get_error_log()))
    ...
    def test_mseed_channel_matcher_normalize(self):
        ...
        matcher = mseed_channel_matcher(self.db, prepend_collection_name=False, cache_normalization_data=False)
    ...
    def test_mseed_site_matcher_get_document(self):
        ...
        assert(ts_1.dead())
        assert(ts_2.dead())
        assert("No match for net_sta_chan_loc =TA_034A and time=" in str(ts_1.elog.get_error_log()))
        assert("No match for query = " in str(ts_2.elog.get_error_log()))
    ...
    def test_mseed_site_matcher_normalize(self):
        ...
        cached_retdoc = matcher(ts_1)
        matcher = mseed_site_matcher(self.db, prepend_collection_name=False, cache_normalization_data=False)
        uncached_retdoc = matcher(ts_2)
        assert(Metadata_cmp(cached_retdoc, uncached_retdoc))
