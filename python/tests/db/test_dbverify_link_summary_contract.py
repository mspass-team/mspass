...
def test_contract_suite_loads_dbverify_from_selected_source():
    source_root = Path(
        os.environ.get("MSPASS_TEST_SOURCE_ROOT", Path(__file__).parents[2])
    )
    expected = source_root / "mspasspy/db/script/dbverify.py"
    assert Path(dbverify.__file__).resolve() == expected.resolve()
