...
def test_filter():
    ...
    filter(ts, "bandpass", freqmin=1, freqmax=5, preserve_history=True, alg_id='0')
    filter(seis, "bandpass", freqmin=1, freqmax=5, preserve_history=True, alg_id='0')
    filter(tse, "bandpass", freqmin=1, freqmax=5, preserve_history=True, alg_id='0')
    filter(seis_e, "bandpass",  freqmin=1, freqmax=5, preserve_history=True, alg_id='0')
...
def test_detrend():
    ...
    tse = get_live_timeseries_ensemble(3)
    seis_e = get_live_seismogram_ensemble(3)
    detrend(ts, preserve_history=True, alg_id='0')
    detrend(seis, preserve_history=True, alg_id='0')
    detrend(tse, preserve_history=True, alg_id='0')
    detrend(seis_e, preserve_history=True, alg_id='0')
    detrend(ts, type="linear", preserve_history=True, alg_id='0')
    detrend(ts, type="constant", preserve_history=True, alg_id='0')
    detrend(ts, type="polynomial", order=2, preserve_history=True, alg_id='0')
    detrend(ts, type="spline", order=2, dspline=1000, preserve_history=True, alg_id='0')
    ...
    detrend(ts, "simple", preserve_history=True, alg_id='0')
    assert all(abs(a-b) < 0.001 for a, b in zip(ts.data, tr.data))
    assert not all(abs(a-b) < 0.001 for a, b in zip(ts.data, copy))
...
def test_interpolate():
    ...
    tse = get_live_timeseries_ensemble(3)
    seis_e = get_live_seismogram_ensemble(3)
    interpolate(ts, 255, preserve_history=True, alg_id='0')
    interpolate(seis, 255, preserve_history=True, alg_id='0')
    interpolate(tse, 255, preserve_history=True, alg_id='0')
    interpolate(seis_e, 255, preserve_history=True, alg_id='0')
    interpolate(ts, 255, method='lanczos', a=20, preserve_history=True, alg_id='0')
    ts = get_live_timeseries()
    interpolate(ts, 25, method='slinear', preserve_history=True, alg_id='0')
    ts = get_live_timeseries()
    interpolate(ts, 255, method='linear', preserve_history=True, alg_id='0')
    ts = get_live_timeseries()
    interpolate(ts, 255, method='nearest', preserve_history=True, alg_id='0')
    ts = get_live_timeseries()
    interpolate(ts, 255, method='zero', preserve_history=True, alg_id='0')
    ...
    interpolate(ts, 40, method='linear', npts=500, preserve_history=True, alg_id='0')
...
def test_correlate():
    ...
    tr1 = ts1.toTrace()
    tr2 = ts2.toTrace()
    res1 = correlate(ts1, ts2, 2, preserve_history=True, alg_id='0')
...
def test_correlate_template():
    ...
    tr1 = ts1.toTrace()
    tr2 = ts2.toTrace()
    res1 = correlate_template(ts1, ts2, preserve_history=True, alg_id='0')
...
def test_correlate_stream_template():
    ...
    st1 = tse1.toStream()
    st2 = tse2.toStream()
    res1 = correlate_stream_template(tse1, tse2, preserve_history=True, alg_id='0')
