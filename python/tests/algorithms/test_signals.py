...
def test_filter():
    ...
    filter(ts, "bandpass", freqmin=1, freqmax=5, object_history=True, alg_id="0")
    filter(seis, "bandpass", freqmin=1, freqmax=5, object_history=True, alg_id="0")
    filter(tse, "bandpass", freqmin=1, freqmax=5, object_history=True, alg_id="0")
    filter(seis_e, "bandpass", freqmin=1, freqmax=5, object_history=True, alg_id="0")
...
def test_detrend():
    ...
    detrend(ts, type="spline", order=2, dspline=1000, object_history=True, alg_id="0")
...
def test_interpolate():
    ...
    interpolate(ts, 255, method="lanczos", a=20, object_history=True, alg_id="0")
    ts = get_live_timeseries()
    ...
    interpolate(ts, 40, method="linear", npts=500, object_history=True, alg_id="0")
...
def test_correlate():
    ...
    tr1 = ts1.toTrace()
    tr2 = ts2.toTrace()
    res1 = correlate(ts1, ts2, 2, object_history=True, alg_id="0")
...
def test_correlate_template():
    ...
    tr1 = ts1.toTrace()
    tr2 = ts2.toTrace()
    res1 = correlate_template(ts1, ts2, object_history=True, alg_id="0")
...
def test_correlate_stream_template():
    ...
    st1 = tse1.toStream()
    st2 = tse2.toStream()
    res1 = correlate_stream_template(tse1, tse2, object_history=True, alg_id="0")
...
def test_xcorr_pick_correction():
    ...
    dt, coeff = obspy.signal.cross_correlation.xcorr_pick_correction(
        t1, tr1, t2, tr2, 0.05, 0.2, 0.1
    )
