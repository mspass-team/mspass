...
def test_topMute():
    ...
    ldata = pickle.dumps(lmute)
    lcopy = pickle.loads(ldata)
    assert math.isclose(lcopy.get_t0(), lmute.get_t0())
    assert math.isclose(lcopy.get_t1(), lmute.get_t1())
    assert "linear" == lcopy.taper_type()
    ...
    cdata = pickle.dumps(cmute)
    ccopy = pickle.loads(cdata)
    assert math.isclose(ccopy.get_t0(), cmute.get_t0())
    assert math.isclose(ccopy.get_t1(), cmute.get_t1())
    assert "cosine" == ccopy.taper_type()
