...
def validate_lags(e, lag_expected, i0=0, klag="arrival_time_correction"):
    ...
    for i in range(20):
        if e.member[i].live:
            print(
                (lag_expected[0] - lag_expected[i]),
                round(e.member[i]["arrival_time_correction"] / e.member[i].dt),
            )
...
def test_dbxcor_weights_preserves_zero_and_dead_sentinels():
    ...
    zero_stack_weights = dbxcor_weights(ensemble, _make_live_timeseries([0.0, 0.0]))
...
def test_align_and_stack():
    ...
    [eo, beam] = align_and_stack(
        e, w, window_beam=True, robust_stack_window=rwin, robust_stack_method="dbxcor"
    )
    ...
    [eo, beam] = align_and_stack(e, w, window_beam=True, robust_stack_method="dbxcor")
    assert count_live(eo) == 20
    print_elog(eo)
    validate_lags(eo, lag_in_samples)
...
def test_MCXcorPrepP():
    ...
    e = TimeSeriesEnsemble(e0)
    [eo, beam] = MCXcorPrepP(
        e, nw, station_collection="site", low_f_corner=0.1, high_f_corner=1.0, npoles=2
    )
