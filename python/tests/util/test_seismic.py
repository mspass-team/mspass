import pytest
from mspasspy.util.seismic import print_metadata, number_live, ensemble_time_range, regularize_sampling
...
def test_print_metadata():
    ...
    x=dict(d)
    print_metadata(x)
    with pytest.raises(TypeError,match="arg0 must be a dictionary or a subclass of Metadata"):
