...
def setup_environment():
    if os.path.exists(wfdir):
        raise Exception(
            "test_distributed setup:  scratch directory={} exists.  Must be empty to run this test".format(
                wfdir
            )
        )
    else:
...
def define_kill_one():
    def kill_one(d):
        ...
        if d["site_sta"] == "station1":
            d.kill()
            d.elog.log_error(
                "kill_one", "test function killed this datum", ErrorSeverity.Invalid
            )
        return d
...
def define_massacre():
    def massacre(d):
        ...
        d.kill()
        d.elog.log_error(
            "massacre", "test function killed this datum", ErrorSeverity.Invalid
        )
        return d
...
def define_kill_one_member():
    def kill_one_member(ens):
        ...
        """
        ens.member[0].kill()
        ens.member[0].elog.log_error(
            "kill_one_member", "test function killed this datum", ErrorSeverity.Invalid
        )
        return ens
...
def test_read_distributed_atomic(
    ...
    source_matcher = ObjectIdMatcher(
        db, "source", attributes_to_load=["lat", "lon", "depth", "time", "_id"]
    )
    nrmlist.append(source_matcher)

    site_matcher = ObjectIdMatcher(
        db, "site", attributes_to_load=["net", "sta", "lat", "lon", "elev", "_id"]
    )
    ...
    new_wfids = write_distributed_data(
        bag_or_rdd,
        db,
...
def test_write_distributed_atomic(
    ...
    test_wfids = write_distributed_data(
        bag_or_rdd,
        db,
        data_are_atomic=True,
        collection=collection,
...
def test_read_distributed_ensemble(
    ...
    wfidlists = write_distributed_data(
        bag_or_rdd,
        db,
...
def test_write_distributed_ensemble(
    ...
    if scheduler == "dask":
        ...
        bag_or_rdd = bag_or_rdd.map(lambda d: kill_one_member(d))
    data_tag = (
        "kill_one_cremation_test"  # made a variable to allow changes to copy code
    )
    wfidlists = write_distributed_data(
        bag_or_rdd,
        db,
    ...
    data_tag = (
        "pedantic_write_kill_test"  # made a variable to allow changes to copy code
    )
    wfidlists = write_distributed_data(
        bag_or_rdd,
        db,
