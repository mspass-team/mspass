...
def _load_normalization_cache(db,collection,
                              required_attributes=[],
                              optional_attributes=[],
                              query={}):
    """
    ...
    for doc in cursor:
        ...
        for key in required_attributes:
            if key in doc:
                ...
                raise MsPASSError("_load_normalization_cache:   required attribute with key = " + key + " not found",
                                  ErrorSeverity.Fatal)
...
class ID_matcher(NMF):
    ...
    def __init__(self, db, 
        ...
        if isinstance(collection,str):
            ...
            if self.cache_normalization_data:
                ...
                self.cache = _load_normalization_cache(db, collection,
    ...
    def get_document(self,d):
        ...
        if d.is_defined(self.mdkey):
            ...
            self.log_error(d,"ID_matcher.get_document",
                        message,self.kill_on_failure,ErrorSeverity.Invalid)
            return None
        if self.cache_normalization_data:
            ...
            for key in self.attributes_to_load:
                if key in doc:
                    ...
                    message = "Required key=" + key + " not found in normalization collection = " + self.collection
                    self.log_error(d,"ID_matcher.get_document",
                        message,self.kill_on_failure,ErrorSeverity.Invalid)
...
class mseed_channel_matcher(NMF):
    ...
    """
    def __init__(self, db, 
                 attributes_to_load=["_id","net", "sta", "chan", "lat","lon","elev","hang","vang","starttime","endtime"],
                 load_if_defined=["loc"],
                 cache_normalization_data=True,
                 query={},
        ...
        if cache_normalization_data:
            self.cache_normalization_data = cache_normalization_data
            # We dogmatically require prepend_collection_name=True 
            self.cache = _load_normalization_cache(db, "channel",
                                            required_attributes=self.attributes_to_load,
                                            optional_attributes=self.load_if_defined,
                                            query=query)
        ...
        if key in self.xref:
            ...
            if test_time == None:
                if error_logging_allowed:
                    ...
                    self.log_error(d,"mseed_channel_matcher._cached_get_document",
                               message,False,ErrorSeverity.Suspect)
                idkey = doclist[0]
                return self.cache[idkey]
            for idkey in doclist:
                ...
                if test_time >= stime and test_time <= etime:
                    return doc
            if error_logging_allowed:
                message = "No match for net_sta_chan_loc =" + key + " and time=" + str(UTCDateTime(test_time))
                self.log_error(d,"mseed_channel_matcher._cached_get_document",
                           message, self.kill_on_failure,ErrorSeverity.Invalid)
            return None
        else:
            if error_logging_allowed:
                message = "No entries are present in channel collection for net_sta_chan_loc = " + key
                self.log_error(d,"mseed_channel_matcher._cached_get_document",
                           message, self.kill_on_failure,ErrorSeverity.Invalid)
            return None
...
class mseed_site_matcher(NMF):
    ...
    """
    def __init__(self, db, 
                 attributes_to_load=["_id","net", "sta", "lat","lon","elev","starttime","endtime"],
                 load_if_defined=["loc"],
                 cache_normalization_data=True,
                 query={},
                 readonly_tag="READONLYERROR_", 
        ...
        if cache_normalization_data:
            self.cache_normalization_data = cache_normalization_data
            # We dogmatically require prepend_collection_name=True 
            self.cache = _load_normalization_cache(db, "site",
                                            required_attributes=self.attributes_to_load,
                                            optional_attributes=self.load_if_defined,
                                            query=query)
            self.xref = self._build_xref()
    ...
    def _cached_get_document(self,d,time=None):
        ...
        if key in self.xref:
            ...
            if test_time == None:
                if error_logging_allowed:
                    message = "Warning - no time specified for match and data has no starttime field defined.  Using first match found in channel collection"
                    self.log_error(d,"mseed_site_matcher._cached_get_document",
                               message,False,ErrorSeverity.Suspect)
                idkey = doclist[0]
                return self.cache[idkey]
            for idkey in doclist:
