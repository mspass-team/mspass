...
class Client:
    ...
    def __init__(self, database_host=None, scheduler=None, scheduler_host=None, job_name='mspass', database_name='mspass', schema=None, collection=None):
        ...
        if database_host is not None and not type(database_host) is str:
            raise MsPASSError('database_host should be a string but ' +
                              str(type(database_host)) + ' is found.', 'Fatal')
        if scheduler is not None and scheduler != 'dask' and scheduler != 'spark':
            raise MsPASSError('scheduler should be either dask or spark but ' +
                              str(scheduler) + ' is found.', 'Fatal')
        if scheduler_host is not None and not type(scheduler_host) is str:
            raise MsPASSError('scheduler_host should be a string but ' +
                              str(type(scheduler_host)) + ' is found.', 'Fatal')
        if job_name is not None and not type(job_name) is str:
            raise MsPASSError('job_name should be a string but ' +
                              str(type(job_name)) + ' is found.', 'Fatal')
        if database_name is not None and not type(database_name) is str:
            raise MsPASSError('database_name should be a string but ' +
                              str(type(database_name)) + ' is found.', 'Fatal')
        # collection should be a string
        if collection is not None and type(collection) is not str:
            raise MsPASSError('collection should be a string but ' +
                              str(type(collection)) + ' is found.', 'Fatal')
        ...
        try:
            ...
            raise MsPASSError(
                'Runntime error: cannot create a database client with: ' + database_address, 'Fatal')
        ...
        if self._scheduler == 'spark':
            ...
            try:
                ...
                raise MsPASSError(
                    'Runntime error: cannot create a spark configuration with: ' + self._spark_master_url, 'Fatal')
            ...
            if not scheduler_host and not MSPASS_SCHEDULER_ADDRESS:
                ...
                try:
                    ...
                    raise MsPASSError(
                        'Runntime error: cannot create a dask client with: ' + self._dask_client_address, 'Fatal')
    ...
    def set_global_history_manager(self, history_db, job_name, collection=None):
        ...
        if not isinstance(history_db, Database):
            raise TypeError('history_db should be a mspasspy.db.Database but ' +
