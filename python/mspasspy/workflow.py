...
def sliding_window_pipeline(dlist, 
                            processing_function, 
                            dask_client, 
    ...
    if not isinstance(dlist,Iterable):
        message = "{}:  Illegal value for arg0 - must be an interable container (usually a list)".format(alg)
        raise ValueError(message)
    if not callable(processing_function):
        message = "{}:  Illegal value for arg1 - must be the name of a processing function to be submitted".format(alg)
        raise ValueError(message)
    if pfunc_args is None:
        ...
        if not isinstance(pfunc_args,Iterable):
            message = "{}:  Illegal input for pfunc_args.   Must be an iterable (usually a list)".format(alg)
            raise ValueError(message)
    if pfunc_kwargs is None:
        pfunc_kwargs = {}
    else:
        if not isinstance(pfunc_kwargs,Mapping):
            message="{}:  Illegal input for pfunc_kwargs.   Must be an dict like container (isinstance(x,Mapping) == True)".format(alg)
            raise ValueError(message) 
    ...
    if completion_function is None:
        ...
        if cfunc_args is None:
            ...
            if not isinstance(cfunc_args,Iterable):
                message = "{}:  Illegal input for cfunc_args.   Must be an iterable (usually a list)".format(alg)
                raise ValueError(message)
        if cfunc_kwargs is None:
            cfunc_kwargs = {}
        else:
            if not isinstance(cfunc_kwargs,Mapping):
                message="{}:  Illegal input for cfunc_kwargs.   Must be an dict like container (isinstance(x,Mapping) == True)".format(alg)
                raise ValueError(message)
        if accumulator is not None:
            ...
            if a_args is None:
                ...
                if not isinstance(a_args,Iterable):
                    message = "{}:  Illegal input for a_args.   Must be an iterable (usually a list)".format(alg)
                    raise ValueError(message)
            if a_kwargs is None:
                a_kwargs = {}
            else:
                if not isinstance(a_kwargs,Mapping):
                    message="{}:  Illegal input for a_kwargs.   Must be an dict like container (isinstance(x,Mapping) == True)".format(alg)
                    raise ValueError(message)
    ...
    if isinstance(dask_client, ddist.Client):
        ...
        if sliding_window_size=="auto":
            ...
            if swsize<num_workers:
                print(f"{alg}:  WARNING set sliding window size to {swsize} which smaller than the number of workers={num_workers}")
                print("This will not use the dask cluster if you are running this job on HPC")
                print("This setting makes sense only if you are running on a cluster service being used by multiple applications")
    else:
        
        message = "{}: Illegal value for arg2.  Must be an instance of dask.distributed.Client".format(alg)
        raise ValueError(message)
    futures_list = list()
    N = len(dlist)
    if verbose:
    ...
    for f in seq:
        ...
        if run_completion_function:
            ...
            if accumulator is None:
                ...
                accumulated_output = accumulator(accumulated_output,f_result,*a_args,**a_kwargs)
        ...
        if i_d < N:
            if verbose:
                print("Submitting item number ",i_d)
            f = dask_client.submit(processing_function,dlist[i_d],*pfunc_args,**pfunc_kwargs)
            seq.add(f)
            i_d += 1
    if accumulator is None:
        return results
