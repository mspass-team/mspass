...
`here <https://www.mongodb.com/docs/manual/data-modeling/>`__).
...
#.  The two core readers in MsPASS
    (:py:meth:`Database.read_data <mspasspy.db.database.Database.read_data>` for serial workflows
    ...
    contain a list of one or more normalization operators.  When applied this
...
database concepts may find it helpful to view a :code:`normalization operator`
...
`here <https://www.mongodb.com/company/blog/technical/generating-globally-unique-identifiers-for-use-with-mongodb>`__.
...
the :py:meth:`index_mseed_file <mspasspy.db.database.Database.index_mseed_file>` method
...
.. code-block:: python
  ...
  print("Number with channel_id set=", retcodes[1])
  print("Number with site_id set=", retcodes[2])
...
.. code-block:: python
  ...
  dbclient = Client()
  db = dbclient.get_database("mydatabase")
  channel_matcher = ObjectIdMatcher(db, collection="channel")
...
Unlike the serial reader, :code:`read_distributed_data` does not accept a
MongoDB cursor.  For atomic data, pass the database handle and use its
``query`` and ``collection`` arguments; the function creates the cursor
internally so that a non-serializable cursor is not sent to workers.
...
.. code-block:: python
  ...
  for srcid in sourceid_list:
      ...
      ensemble = db.read_data(
          ...
          normalize=[channel_matcher],
          collection="wf_miniseed",
          ensemble_metadata={"source_id": srcid},
      )
...
Note that we used a different operation to handle the ``source`` collection
...
container instead of each ensemble member.  The ``normalize`` reader
argument applies ``channel_matcher`` to each member.  The
``ensemble_metadata`` argument posts the common ``source_id`` to the
ensemble, after which the explicit :py:func:`normalize
<mspasspy.db.normalize.normalize>` call loads the source attributes into the
ensemble's Metadata.

.. note::
  The ``normalize_ensemble`` reader argument was added in version 2 of
  MsPASS.  The distributed ensemble example below shows its current use with
  ``container_to_merge``.  The serial example above uses the explicit
  post-read operation because the serial reader currently applies
  ``ensemble_metadata`` after its normalization step.
...
Note that we set ``handles_ensembles`` False to have the normalize
function process all ensemble members.  Normal behavior for that function
...
.. note::
  The ``handles_ensembles`` wrapper argument replaces the older
  ``apply_to_members`` spelling.  Set it False to apply an atomic matcher to
  each member; leave it True (the default) to normalize ensemble Metadata.
...
to implement normalization is via a caching algorithm.  That is, we
...
#.  :py:class:`DataFrameCacheMatcher <mspasspy.db.normalize.DataFrameCacheMatcher>`
    ...
    to store its internal cache.  The pandas library is robust and
...
#.  Both can load the normalizing collection in one of two forms: (a)
    ...
    `pandas read_csv method <https://pandas.pydata.org/docs/reference/api/pandas.read_csv.html>`__
...
matcher classes that utilize a caching method.  As above each name
...
.. list-table:: Cache-based Matchers
   ...
   * - Class Name
     - Use
   * - :py:class:`ObjectIdMatcher <mspasspy.db.normalize.ObjectIdMatcher>`
     - Match with MongoDB ObjectId as dictionary key for cache
   * - :py:class:`MiniseedMatcher <mspasspy.db.normalize.MiniseedMatcher>`
     - miniSEED match with net:sta:chan:loc and time
...
.. code-block:: python
  ...
  matcher = ObjectIdMatcher(
      db,
      collection="site",
      attributes_to_load=attribute_list,
  )
  # This says load the entire dataset presumed staged to MongoDB
  dataset = read_distributed_data(db, collection="wf_TimeSeries")
...
calling :code:`read_distributed_data` with a matcher for site in the normalize list.
...
+  Only the attributes defined in the constructor for the matcher are copied
   ...
   after running the normalize function each datum for which a match
   was found will contain attributes with the following keys:
   :code:`site_id`, :code:`site_lat`, :code:`site_lon`, and :code:`site_elev`.
   Note these have the string "site\_" automatically prepended by default.
   That renaming can be disabled by setting :code:`prepend_collection_name`
...
symbols :code:`channel_attribute_list` and :code:`site_attribute_list`.
...
.. code-block:: python
  ...
  dbclient = Client()
  db = dbclient.get_database("mydatabase")
  # starttime and endtime are required by MiniseedMatcher's interval test.
  channel_attribute_list = [
      "_id", "starttime", "endtime", "lat", "lon", "elev", "hang", "vang"
  ]
  site_attribute_list = [
      "_id", "starttime", "endtime", "lat", "lon", "elev"
  ]
  # These construct the channel and site normalizers.
  channel_matcher = MiniseedMatcher(
      db,
      collection="channel",
      attributes_to_load=channel_attribute_list,
  )
  site_matcher = MiniseedMatcher(
      db,
      collection="site",
      attributes_to_load=site_attribute_list,
  )
  dataset = read_distributed_data(db, collection="wf_miniseed")
  dataset = dataset.map(normalize, channel_matcher)
...
If it is not defined it will be silently ignored and left undefined.  Note
...
.. code-block:: python
  ...
  from mspasspy.client import Client
  from mspasspy.db.normalize import OriginTimeMatcher, normalize
  from mspasspy.io.distributed import read_distributed_data
  ...
  source_matcher = OriginTimeMatcher(
      db,
      t0offset=20.0,
      attributes_to_load=attribute_list,
      load_if_defined=["magnitude"],
  )
  dataset = read_distributed_data(db, collection="wf_Seismogram")
...
calling :py:func:`read_distributed_data
<mspasspy.io.distributed.read_distributed_data>` with that query list
to create the ensembles.  The resulting bag of ensembles is then normalized.
...
.. code-block:: python
  ...
  def srcidlist2querylist(srcidlist):
    ...
    read_distributed_data accepts a list of Python dictionaries
    assumed to be valid MongoDB queries.
    """
    return [{"source_id": srcid} for srcid in srcidlist]
  ...
  source_ids = dbag.from_sequence(
      [{"source_id": srcid} for srcid in sourceid_list]
  )
  dataset = read_distributed_data(
      querylist,
      db=db,
      collection="wf_Seismogram",
      container_to_merge=source_ids,
...
.. code-block:: python
  ...
  class MyCustomMatcher(DataFrameCacheMatcher):
    def __init__(
      self,
      db,
      # additional required arguments without defaults would be defined here
      collection="site",
      ...
      # parse any additional arguments and set instance variables here
...
that define properties of the matching algorithm you are implementing.
...
from the :code:`BasicMatcher` family is that you can then use it in a
map operator to load attributes from a normalizing collection within a
