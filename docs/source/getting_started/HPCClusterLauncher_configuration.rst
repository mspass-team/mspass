...
Overview
-------------------------
This document gives details of configuration parameters
needed to launch a valid instance of the
:code:`HPCClusterLauncher` object.  The parameters have two groups:
...
``HPCClusterLauncher`` is distributed by the separate
`mspass_launcher project
<https://github.com/mspass-team/mspass_launcher>`__.  Start with the template
shipped with the same launcher version that will run the job.  The latest
published release is 0.1.1, while the development branch has a different
configuration schema.  Most required keys are accessed directly, so a
missing or misspelled key normally raises ``KeyError``.
...
- *worker_run_command* is the MPI launcher command used by release 0.1.1,
  normally ``mpiexec``.  The development branch replaces this key with
  *remote_worker_run_command*; use only the key required by the installed
  launcher version.
...
Development-branch MPI parameters
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

The current development branch adds the following ``HPC_cluster`` keys.  They
are not recognized by release 0.1.1:

- *remote_worker_run_command* replaces *worker_run_command*.
- *mpi_implementation* is required and must be ``openmpi``, ``mpich``, or
  ``tacc``.  It controls the generated host-file format.
- *hostlist_filename* is required and names a writable host file generated by
  the launcher and passed to the MPI command with ``-hostfile``.
- *mpi_arg_list* is an optional YAML sequence inserted after ``-hostfile`` and
  its filename.
- *remote_processes_per_node* is optional.  When set, it adds per-node slot
  counts for Open MPI or MPICH.  The ``tacc`` implementation ignores it.
