...
RUN ln -s /usr/local/spark/python/pyspark /opt/conda/lib/python3.10/site-packages/pyspark
RUN unzip /usr/local/spark/python/lib/py4j-0.10.9-src.zip -d /opt/conda/lib/python3.10/site-packages/ \
	&& docker-clean
...
RUN pip3 --no-cache-dir install jedi==0.17.2 notebook==6.2.0 && docker-clean
...
RUN sed -i 's/localhost:27020,/127.0.0.1:27020,/g' /opt/conda/lib/python3.10/site-packages/pymongo/encryption_options.py && \
	sed -i 's/HOST = "localhost"/HOST = "127.0.0.1"/g' /opt/conda/lib/python3.10/site-packages/pymongo/mongo_client.py && \
	sed -i "s/'localhost'/'127.0.0.1'/g" /opt/conda/lib/python3.10/site-packages/pymongo/settings.py && \
	sed -i "s/'localhost'/'127.0.0.1'/g" /opt/conda/lib/python3.10/site-packages/pymongo/pool.py
