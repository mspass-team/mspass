  ...
  else
  {
    ...
    if(instance>=(aptr->second.size()))
    {
      cerr << "ProcessManager::algorithm(Warning):  "
        << "algorithm="<<name<<" was defined but requested instance="
        << instance<<" exceeds number of defined instances="
        << aptr->second.size()<<endl
        << "Set to id for instance="
        << aptr->second.size() - 1<<endl
