...
AttributeProperties::AttributeProperties(const string st) {
  ...
  const string emess("AttributeProperties(string) constructor failure:  "
                     "failure parsing the following string:\n");

  if (st.empty())
    throw MsPASSError(string("AttributeProperties constructor failure;  "
                             "Constructor was passed an empty string\n"),
  ...
  if (end_current < 0)
    throw MsPASSError(string(emess + st));
  ...
  if (mdtype_word == "REAL" || mdtype_word == "real")
    ...
    mdt = MDtype::Invalid;
    throw MsPASSError(string("AttributeProperties(string) constructor:  "
  ...
  if (current >= 0) {
    ...
    if (bool_word == "yes" || bool_word == "true" || bool_word == "TRUE")
      is_key = true;
  }
}
...
AttributeProperties &
AttributeProperties::operator=(const AttributeProperties &apin) {
  if (&apin == this)
    return (*this);
  db_attribute_name = apin.db_attribute_name;
  db_table_name = apin.db_table_name;
...
ostream &operator<<(ostream &ofs, const AttributeProperties &d) {
  ofs << d.db_attribute_name << " " << d.db_table_name << " "
      << d.fully_qualified_name() << " " << d.internal_name << " ";
  switch (d.mdt) {
  case MDtype::Integer:
    ofs << "integer" << endl;
    break;
...
AttributeMap::AttributeMap(const AntelopePf &pf, const string name) {
  ...
  try {
    ...
    if (attbl.size() <= 0)
      throw MsPASSError(base_error + "Attributes table is empty. Is parameter "
                                     "file may be missing Attributes Tbl?");
...
map<string, AttributeProperties> AttributeMap::aliases(const string key) const {
  ...
  if (!this->is_alias(key))
    throw MsPASSError("AttributeMap::aliases method: " + string("Attribute ") +
                      key + string(" is not defined as an alias"));
  else {
    ...
    for (listiter = aml.begin(); listiter != aml.end(); ++listiter) {
      ...
      if (amiter == attributes.end())
        throw MsPASSError(
            "AttributeMap::aliases method: " + string("Attribute named ") +
            (*listiter) + string(" is not defined for this AttributeMap"));
...
list<string> AttributeMap::aliastables(const string key) const {
  ...
  if (!this->is_alias(key))
    throw MsPASSError(
        "AttributeMap::aliastables method: " + string("Attribute ") + key +
    ...
    for (listiter = aml.begin(); listiter != aml.end(); ++listiter) {
      ...
      if (amiter == attributes.end())
        throw MsPASSError(
            "AttributeMap::aliastables method: " + string("Attribute named ") +
            (*listiter) + string(" is not defined for this AttributeMap"));
...
AttributeProperties AttributeMap::operator[](const std::string key) const {
  ...
  if (amptr == attributes.end()) {
    throw MsPASSError(string("AttributeMap::opertor[]: key=") + key +
                      " has not entry in this AttributeMap");
  } else {
    return amptr->second;
  }
}
