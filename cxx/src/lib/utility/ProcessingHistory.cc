...
bool NodeData::operator!=(const NodeData &other) { return !((*this) == other); }
...
ProcessingHistory::ProcessingHistory() : elog() {
  ...
  current_stage =
      -1; // illegal value that could be used as signal for uninitalized
...
bool ProcessingHistory::is_empty() const {
  if ((current_status == ProcessingStatus::UNDEFINED) && (nodes.empty()))
    return true;
  return false;
}
...
size_t ProcessingHistory::number_of_stages() { return current_stage; }
...
void ProcessingHistory::set_as_origin(const string alg, const string algid_in,
  ...
  if (nodes.size() > 0) {
    elog.log_error(alg + ":" + algid_in,
                   base_error + "Illegal usage.  History chain was not empty.  "
                                " Calling clear method and continuing",
...
string ProcessingHistory::new_ensemble_process(
    const string alg, const string algid_in, const AtomicType typ,
    const vector<ProcessingHistory *> parents, const bool create_newid) {
  if (create_newid) {
    this->newid();
  }
  ...
  for (i = 0; i < parents.size(); ++i) {
    ...
    if (nd.stage > max_stage)
      max_stage = nd.stage;
    for (nptr = parent_node_data.begin(); nptr != parent_node_data.end();
         ++nptr) {
  ...
  if (current_stage >= 0)
    ...
    elog.log_error("ProcessingHistory::new_ensemble_process",
                   "current_stage for none of the parents was "
                   "initialized\nImproper usage will create an invalid history "
                   "chain that may cause downstream problems",
                   ErrorSeverity::Complaint);
    current_stage = 0;
  }
  algorithm = alg;
...
/* Companion to new_ensemble_process that appends the history of one datum to
the multimap containers.  It does not alter the current values the
new_ensemble_process method MUST have been called before calling this method or
the history chain will become corrupted.*/
void ProcessingHistory::add_one_input(const ProcessingHistory &data_to_add) {

  if (data_to_add.is_empty()) {
    stringstream ss;
    ss << "Data with uuid=" << data_to_add.id() << " has an empty history chain"
       << endl
    ...
    a node if the data are unique.  This is simple compared to
    new_ensemble_process because we just have to check one object's history at a
    time. */
...
string ProcessingHistory::new_map(const string alg, const string algid_in,
  ...
  if (this->is_empty()) {
    ...
    ss << "Attempt to call this method on an empty history chain for uuid="
       << this->id() << endl
       << "Cannot preserve history for algorithm=" << alg
       << " with id=" << algid << endl;
    elog.log_error("ProcessingHistory::new_map", ss.str(),
                   ErrorSeverity::Complaint);
    return current_id;
  }
  ...
  algid = algid_in;
  current_status =
      newstatus; // Probably should default in include file to VOLATILE
  if (current_stage >= 0)
    ++current_stage;
  else {
    elog.log_error(
        ...
        "create an invalid history chain that may cause downstream problems",
        ErrorSeverity::Complaint);
    current_stage = 0;
  }
  mytype = typ;
...
string ProcessingHistory::map_as_saved(const string alg, const string algid_in,
  ...
  if (this->is_empty()) {
    ...
    ss << "Attempt to call this method on an empty history chain for uuid="
       << this->id() << endl
       << "Cannot preserve history for writer=" << alg << " with id=" << algid
       << endl;
    elog.log_error("ProcessingHistory::map_as_saved", ss.str(),
                   ErrorSeverity::Complaint);
    return current_id;
  }
...
void ProcessingHistory::accumulate(const string algin, const string algidin,
  ...
  names and ids that mark the current process define this reduce operation*/
    ...
    have to push this's history data to convert it's current data to define an
    input. That means getting a new uuid and pushing current node data to the
    nodes map as an input */
    NodeData nd;
    nd = this->current_nodedata();
    this->newid();
    pair<string, NodeData> pn(current_id, nd);
...
string ProcessingHistory::clean_accumulate_uuids() {
  ...
  for (auto nptr = this->nodes.begin(); nptr != this->nodes.end(); ++nptr) {
    ...
    /* this depends upon the distinction between set and multiset.  i.e. an
    insert of a duplicate does nothing*/
  ...
  if (matching_ids.size() == 1)
    ...
    for (auto sptr = need_to_erase.begin(); sptr != need_to_erase.end();
         ++sptr) {
      nodes.erase(*sptr);
    }
    need_to_erase.clear();
...
void ProcessingHistory::set_id(const string newid) { this->current_id = newid; }
...
list<string> algorithm_outputs(const ProcessingHistory &h, const string alg,
  ...
  for (hptr = hmap.begin(); hptr != hmap.end(); ++hptr) {
    ...
    if ((alg == n.algorithm) && (aid == n.algid))
      result.push_back(hptr->first);
  }
  return result;
}
