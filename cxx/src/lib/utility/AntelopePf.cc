...
AntelopePf pfread(const string fname) {
  ...
  while (inp.getline(inbuffer, 512)) {
    ...
    if (rawline.find_first_not_of(white, 0) == string::npos)
      continue;
    if (is_comment_line(rawline))
      continue;
...
AntelopePf::AntelopePf(const AntelopePf &parent) : Metadata(parent) {
