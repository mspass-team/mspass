...
MetadataDefinitions::MetadataDefinitions(const std::string mdname) {
  try {
    ...
    /* We throw an exception if the name is a path with / characters*/
      ...
      throw MsPASSError("MetadataDefinitions:  name passed seems to be a full "
                        "path name that is not allowed\nReceived this:  " +
                            mdname,
...
MetadataDefinitions::MetadataDefinitions(const string fname,
  ...
  try {
    ...
    case MDDefFormat::PF:
      ...
      break;
    default:
      throw MsPASSError("MetadataDefinitions file constructor:   illegal "
...
MetadataDefinitions::MetadataDefinitions(const MetadataDefinitions &parent)
    : tmap(parent.tmap), cmap(parent.cmap), aliasmap(parent.aliasmap),
...
void MetadataDefinitions::add(const std::string key, const std::string concept_,
                              const MDtype mdt) {
  cmap[key] = concept_;
  tmap[key] = mdt;
}
...
void MetadataDefinitions::add_alias(const std::string key,
...
MetadataDefinitions::unique_name(const string aliasname) const {
  ...
  if (aptr == alias_xref.end()) {
    throw MsPASSError(base_error + "alias name=" + aliasname +
                          " is not defined",
                      ErrorSeverity::Invalid);
  } else {
...
MetadataDefinitions::operator+=(const MetadataDefinitions &other) {
  ...
  for (kptr = kvals.begin(); kptr != kvals.end(); ++kptr) {
    ...
    /* Note this will silently overwrite previous if the key was already
     * present*/
    this->tmap[*kptr] = mdt;
    try {
      ...
      cerr << "MetadataDefinitions operator+= (Warning): concept description "
              "is missing for key="
           << *kptr << endl
           << "Error will be ignored" << endl;
    }
    /* The alias_xref is a multimap so we just append other data - no such thing
     * as duplicates*/
    map<string, string>::const_iterator aptr;
    for (aptr = other.alias_xref.begin(); aptr != other.alias_xref.end();
         ++aptr) {
    ...
    for (uptr = other.unique_id_data.begin();
         uptr != other.unique_id_data.end(); ++uptr) {
...
MetadataDefinitions::normalize_data(const string key) const {
  map<string, tuple<string, string>>::const_iterator uidptr;
  uidptr = unique_id_data.find(key);
  if (uidptr == unique_id_data.end()) {
    throw MsPASSError("MetadataDefinitions::normalize_data:  key=" + key +
                      " has no normalization data");
  } else {
...
MDtype str2mdt(string tstr) {
  ...
  /* in the 64 bit world we default int and integer to int64 */
  else if ((tstr == "long") || (tstr == "int64") || (tstr == "int") ||
           (tstr == "integer"))
    ...
    throw MsPASSError("MetadataDefinitions::pfreader:  type value=" + tstr +
                      " not recognized");
  return mdt_this;
}
/* Private methods */
void MetadataDefinitions::pfreader(const string pfname) {
  try {
    ...
    normally create this thing near the start of execution so aborting on
    failure is the expected norm */
        ...
        if (attributes[i]["aliases"]) {
          ...
          if (str.size() > 0) {
            ...
            while (p != NULL) {
              ...
              ;
              p = strtok(NULL, delim.c_str()); // strtok oddity of NULL meaning
                                               // use last position
            }
            free(s);
          }
...
std::list<string>
MetadataDefinitions::apply_aliases(Metadata &d,
                                   const std::list<string> aliaslist) {
