  ...
  try {
    ...
    for (tptr = t.begin(); tptr != t.end(); ++tptr) {
      ...
      if (mdtype == "real" || mdtype == "REAL" || mdtype == "float" ||
        ...
        stringstream ss;
        ss << "get_mdlist:  Error in mdlist specification type name defined by "
              "key = "
           << mdtype << " was not recognized." << endl
           << "Listed with attribute name=" << mdname << endl
           << "List is not valid." << endl;
