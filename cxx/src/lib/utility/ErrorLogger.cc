...
LogData::LogData(const int jid, const std::string alg, const std::string msg,
...
ostream &operator<<(ostream &ofs, LogData &ld) {
  ...
  ofs << ld.job_id << " " << ld.p_id << " " << ld.algorithm << " " << ld.message
      << endl;
  return ofs;
}
...
ErrorLogger &ErrorLogger::operator+=(const ErrorLogger &other) {
  if (this != &other) {
    for (auto lptr = other.allmessages.begin(); lptr != other.allmessages.end();
         ++lptr) {
...
int ErrorLogger::log_error(
    const std::string alg, const std::string mess,
