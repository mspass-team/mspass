...
SlownessVector::SlownessVector(const double ux0, const double uy0,
                               const double az0) {
  ux = ux0;
  uy = uy0;
  azimuth0 = az0;
...
SlownessVector::operator+(const SlownessVector &other) const {
...
// memory management
double SlownessVector::mag() const noexcept { return (hypot(ux, uy)); }
