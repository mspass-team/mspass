...
void BasicTimeSeries::rtoa() {
  ...
  const string base_error(
      "BasicTimeSeries::rtoa() t0shift for conversion is not defined.");
  ...
  if (t0shift_is_valid || (t0shift > 100.0)) {
    ...
    throw MsPASSError(base_error +
                          "time shift to return to UTC time is not defined",
...
double BasicTimeSeries::time_reference() const {
  ...
  if (t0shift_is_valid)
    ...
    throw MsPASSError(
        base_error + "cannot return time reference as it is marked invalid",
        ErrorSeverity::Invalid);
}
std::vector<double> BasicTimeSeries::time_axis() const {
  ...
  for (int i = 0; i < nsamp; ++i)
    t.push_back(mt0 + this->mdt * static_cast<double>(i));
  return t;
}
} // namespace mspass::seismic
