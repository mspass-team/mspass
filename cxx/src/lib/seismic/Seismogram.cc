...
Seismogram::Seismogram(const size_t nsamples)
  ...
  /* Note this constructor body needs no content.  Just a wrapper for
   * CoreSeismogram */
}
...
Seismogram::Seismogram(const CoreSeismogram &d, const string alg)
  ...
  this->ProcessingHistory::set_as_origin(alg, id, id, AtomicType::SEISMOGRAM,
                                         false);
...
Seismogram::Seismogram(const BasicTimeSeries &b, const Metadata &m,
                       const ProcessingHistory &his, const bool card,
                       const bool ortho, const dmatrix &tm, const dmatrix &uin)
...
Seismogram::Seismogram(const Metadata &md, const string jobname,
  ...
  try {
    ...
    thisid = this->newid();
    this->elog.log_error(algname,
                         "uuid not defined.\nSet by constructor to" + thisid,
  ...
  try {
    ...
    this->elog.log_error(algname,
                         "rawdata boolean not found - default to false",
                         ErrorSeverity::Complaint);
    mark_as_raw = false;
  }
...
size_t Seismogram::memory_use() const {
  ...
  memory_estimate +=
      memory_constants::KEY_AVERAGE_SIZE * this->changed_or_set.size();
  /* Similar for history and elog containers */
  memory_estimate +=
      memory_constants::HISTORYDATA_AVERAGE_SIZE * this->nodes.size();
