...
long int fwrite_sample_data(const string dir, const string dfile, double *dptr,
                            const size_t nd) {
  try {
    FILE *fp;
    long int foff;
    ...
    if ((fp = fopen(fname.c_str(), "a")) == NULL)
      /* use the name of the overloaded parent instead of the actual function -
       * intentional*/
      throw MsPASSError("fwrite_to_file:  Open failed on file " + fname,
    ...
    if (fwrite((void *)dptr, sizeof(double), nd, fp) != nd) {
      fclose(fp);
      throw MsPASSError("fwrite_to_file:  fwrite error while writing to file " +
                            fname,
                        ErrorSeverity::Invalid);
    }
    fclose(fp);
...
fwrite_to_file(mspass::seismic::LoggingEnsemble<mspass::seismic::TimeSeries> &d,
  ...
  try {
    ...
    /* This will return an empty vector if the ensemble is marked dead - callers
    should handle this condition but they normally shouldn't be calling this
    function if the entire ensemble is marked dead anyway.*/
    if (d.dead())
      return (foffs);
    string fname;
    if (dir.length() > 0)
    ...
    if ((fp = fopen(fname.c_str(), "a")) == NULL)
       ...
       * intentional*/
      throw MsPASSError(
          "fwrite_to_file (TimeSeriesEnsemble):  Open failed on file " + fname,
          ErrorSeverity::Invalid);
    ...
    for (int i = 0; i < d.member.size(); ++i) {
      if (d.member[i].dead())
        ...
        if (fwrite((void *)t.s.data(), sizeof(double), t.npts(), fp) !=
            t.npts()) {
          fclose(fp);
          stringstream ss;
          ss << "fwrite_to_file (TimeSeriesEnsemble):  fwrite error while "
                "writing ensemble member "
             << i << " to file=" << fname << endl;
          throw MsPASSError(ss.str(), ErrorSeverity::Invalid);
        }
        /* We always set these 3 attributes in Metadata so they can be properly
        saved to the database after a successful write.  Repetitious with
        Seismogram but a function to do this would be more confusing that
        helpful */
...
fwrite_to_file(mspass::seismic::LoggingEnsemble<mspass::seismic::Seismogram> &d,
  ...
  try {
    ...
    if ((fp = fopen(fname.c_str(), "a")) == NULL)
       ...
       * intentional*/
      throw MsPASSError(
          "fwrite_to_file (SeismogramEnsemble):  Open failed on file " + fname,
          ErrorSeverity::Invalid);
    /* This guarantees appending - not essential since we open in "a" mode but
    clearer */
    fseek(fp, 0L, 2);
    ...
    for (int i = 0; i < d.member.size(); ++i) {
      if (d.member[i].dead())
        ...
        if (fwrite((void *)t.u.get_address(0, 0), sizeof(double), 3 * t.npts(),
                   fp) != 3 * t.npts()) {
          fclose(fp);
          stringstream ss;
          ss << "fwrite_to_file (SeismogramEnsemble):  fwrite error while "
                "writing ensemble member "
             << i << " to file=" << fname << endl;
          throw MsPASSError(ss.str(), ErrorSeverity::Invalid);
        }
...
size_t fread_sample_data(double *buffer, const string dir, const string dfile,
  ...
  if ((fp = fopen(fname.c_str(), "r")) == NULL)
    throw MsPASSError("fread_data_from_file:  Open failed on file " + fname,
                      ErrorSeverity::Invalid);
  if (foff > 0) {
    if (fseek(fp, foff, SEEK_SET)) {
      fclose(fp);
      throw MsPASSError("fread_data_from_file:  fseek failure on file=" + fname,
                        ErrorSeverity::Invalid);
    }
  }
  size_t retcount;
...
size_t fread_from_file(Seismogram &d, const string dir, const string dfile,
  ...
  size_t ns_read;
  try {
    ns_read = fread_sample_data(d.u.get_address(0, 0), dir, dfile, foff,
                                3 * d.npts());
    return ns_read;
  } catch (...) {
    throw;
...
size_t fread_from_file(
  ...
  for (int ind = 0; ind < n; ++ind) {
    ...
    try {
      ...
      ns_read = fread((void *)de.member[i].u.get_address(0, 0), sizeof(double),
                      3 * de.member[i].npts(), fp);
...

{
  ...
  for (int ind = 0; ind < n; ++ind) {
    ...
    try {
      ...
      ns_read = fread((void *)(&(de.member[i].s[0])), sizeof(double),
                      de.member[i].npts(), fp);
