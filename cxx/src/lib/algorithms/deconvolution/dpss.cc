...
void compute_energy_concentrations(double *h, int n, double NW, double *lambda,
  ...
  for (int ii = 1; ii < n; ii++) {
    kern[ii] = sin(2.0 * M_PI * NW / n * ii) / (M_PI * ii);
  }
...
void eig_iit(int n, double *D, double *E, int il, int iu, double *eig_val,
  ...
  char range = 'I', order = 'B',
       safe = 'S'; // index range of eigenvalues, using block splitting and safe
                   // tolerance
  ...
  dstebz_(&range, &order, &n, &vl, &vu, &il, &iu, &abstol, D, E, &m, &nsplit,
          eig_val, IBLOCK, ISPLIT, WORK, IWORK, &info);
  if (info == 0) {
    dstein_(&n, D, E, &m, eig_val, IBLOCK, ISPLIT, eig_vec, &ldz, WORK, IWORK,
            IFAIL, &info);
  }
  // cleanup
  delete[] IBLOCK;
...
void polarize_dpss(double *h, int n, int iseq) {
  ...
  // force positive mean for symmetric sequences, positive first lobe for
  // antisymmetric
  int odd = (iseq % 2);
...
void dpss_calc(int n, double NW, int seql, int sequ, double *h) {
  ...
  double cc = cos(2.0 * M_PI * NW / n) / 4.0;
  ...
  if (nseq_even > 0)
    eig_iit(n_even, D, E, n_even - sequ_even, n_even - seql_even, eig_val,
            &h[start_even], 2 * n);

  // ODD PROBLEM
  D[n_even - 1] = dc - ec;
  if (nseq_odd > 0)
    eig_iit(n_odd, D, E, n_odd - sequ_odd, n_odd - seql_odd,
            &eig_val[nseq_even], &h[start_odd], 2 * n);
...
ERR::ERR(const char *errmsg) { strncpy(msg, errmsg, 30); }
void ERR::getmsg(char *errmsg) { strncpy(errmsg, this->msg, 30); }
const char *ERR::getmsg() { return msg; }
