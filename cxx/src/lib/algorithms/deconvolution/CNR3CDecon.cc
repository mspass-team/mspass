...
CNR3CDecon::CNR3CDecon()
    : FFTDeconOperator(), signalengine(), waveletengine(), dnoise_engine(),
      wnoise_engine(), shapingwavelet(), psnoise(), psnoise_data(), decondata(),
      wavelet() {
...
CNR3CDecon::CNR3CDecon(const AntelopePf &pf)
    : FFTDeconOperator(pf), signalengine(), waveletengine(), dnoise_engine(),
      wnoise_engine(), shapingwavelet(pf), psnoise(), psnoise_data(),
      decondata(), wavelet() {
  // DEBUG
...
void CNR3CDecon::read_parameters(const AntelopePf &pf) {
  try {
    ...
    if (!((swname == "ricker") || (swname == "butterworth"))) {
      throw MsPASSError(
          string("CNR3CDecon(AntelopePf constructor):  ") +
...
CNR3CDecon::CNR3CDecon(const CNR3CDecon &parent)
    : processing_window(parent.processing_window),
      noise_window(parent.noise_window), signalengine(parent.signalengine),
      waveletengine(parent.waveletengine), dnoise_engine(parent.dnoise_engine),
      wnoise_engine(parent.wnoise_engine),
      shapingwavelet(parent.shapingwavelet), psnoise(parent.psnoise),
      psnoise_data(parent.psnoise_data), pssignal(parent.pssignal),
      pswavelet(parent.pswavelet), decondata(parent.decondata),
      wavelet(parent.wavelet), wavelet_taper(parent.wavelet_taper),
      data_taper(parent.data_taper), ao_fft(parent.ao_fft),
      wavelet_bwd(parent.wavelet_bwd), signal_bwd(parent.signal_bwd),
...
from elog of d */
int CNR3CDecon::TestSeismogramInput(Seismogram &d, const int wcomp,
                                    const bool loadnoise) {
  ...
  if (d.time_is_UTC()) {
    stringstream ss;
    ss << base_error << "Data received are using UTC standard; must be Relative"
       << endl;
  ...
  if ((wcomp < 0 || wcomp > 2) && (wcomp != -9999)) {
    stringstream ss;
    ss << base_error << "Illegal component =" << wcomp
       << " specified for wavelet" << endl
  ...
  if ((abs(d.dt() - operator_dt) / operator_dt) > DTSKEW) {
    ...
    ss << base_error << "Mismatched sample intervals.  "
       ...
       << "operator sample interval=" << operator_dt
       << " but data sample interval=" << d.dt() << endl;
    d.elog.log_error("CNR3CDecon", ss.str(), ErrorSeverity::Complaint);
    ++error_count;
  }
  if (this->processing_window.start < d.t0() ||
      this->processing_window.end > d.endtime()) {
    stringstream ss;
    ss << base_error << "Data time window mistmatch." << endl
       << "Data span relative time range =" << d.t0() << " to " << d.endtime()
       << endl
       << "Processing window range of " << this->processing_window.start
       << " to " << this->processing_window.start << " is not inside data range"
       << endl;
    d.elog.log_error("CNR3CDecon", ss.str(), ErrorSeverity::Invalid);
    ++error_count;
  }
  if (loadnoise) {
    if (this->noise_window.start < d.t0() ||
        this->noise_window.end > d.endtime()) {
      stringstream ss;
      ss << base_error << "Noise time window mistmatch." << endl
         << "Data span relative time range =" << d.t0() << " to " << d.endtime()
         << endl
         << "Noise window range of " << this->noise_window.start << d.t0()
         << " to " << this->noise_window.start << " is not inside data range"
         << endl;
...
void CNR3CDecon::loaddata(Seismogram &d, const int wcomp,
  ...
  try {
    if (d.dead())
      throw MsPASSError("CNR3CDecon::loaddata method received data marked dead",
                        ErrorSeverity::Invalid);
...
void CNR3CDecon::loaddata(Seismogram &d, const bool nload) {
  ...
  try {
    ...
    if (errcount > 0) {
      stringstream ss;
      ss << "CNR3CDecon::loaddata:  " << errcount
         << " errors were detected in this call" << endl
         << "Check error log of input Seismogram for detailed error messages"
         << endl
    ...
    */
    ...
    signal_bwd = EstimateBandwidth(
        FFTDeconOperator::df(this->operator_dt), pssignal, psnoise_data,
        snr_bandwidth, signalengine.time_bandwidth_product(), this->fhs);
  } catch (...) {
    throw;
  };
}
/* Note we intentionally do not trap nfft size mismatch in this function because
 * we assume loadwavelet would be called within loaddata or after calls to
 * loaddata
 * */
void CNR3CDecon::loadwavelet(const TimeSeries &w) {
  if (w.dead())
    throw MsPASSError(
        "CNR3CDecon::loadwavelet method received data marked dead",
        ErrorSeverity::Invalid);
  if (w.npts() <= 0)
    throw MsPASSError("CNR3CDecon::loadwavelet method received an empty "
                      "TimeSeries (number samples <=0)",
                      ErrorSeverity::Invalid);
  try {
    ...
    if (this->waveletengine.taper_length() != (w.npts())) {
      this->waveletengine = MTPowerSpectrumEngine(
          w.npts(), this->waveletengine.time_bandwidth_product(),
    ...
    this->wavelet_bwd = EstimateBandwidth(
        FFTDeconOperator::df(this->operator_dt), pswavelet, psnoise,
        snr_bandwidth, waveletengine.time_bandwidth_product(), this->fhs);
    ...
    if (ntest > (this->winlength)) {
      ...
      stringstream ss;
      ss << "loadwavelet method:  size mismatch.  Wavelet received has "
         << ntest << " samples with time>0.0" << endl
         << "This is larger than the processing window length of "
         << this->winlength << endl
         << "Wavelet must be contained in the processing window size" << endl
         << "Truncated on the right to processing window length - results may "
            "be invalid"
         << endl;
    ...
    */
    ...
    // DEBUG
    // cout << "t0 of wavelet stored now wavelet data
    // objec="<<wavelet.t0()<<endl;
    ...
    /* This was the old code fixed Dec 2020 - remove when revision is know to
    work
...
void CNR3CDecon::loadnoise_data(const Seismogram &n) {
  if (n.dead())
    throw MsPASSError(
        "CNR3CDecon::loadnoise_data method received data marked dead",
        ErrorSeverity::Invalid);
  try {
...
void CNR3CDecon::loadnoise_wavelet(const TimeSeries &n) {
  ...
  if (n.dead())
    throw MsPASSError(
        "CNR3CDecon::loadnoise_wavelet method received data marked dead",
        ErrorSeverity::Invalid);
  if (n.npts() <= 0)
    throw MsPASSError(
        "CNR3CDecon::loadnoise_wavelet method received an empty data vector.",
        ErrorSeverity::Invalid);
  try {
    if (n.npts() != wnoise_engine.taper_length()) {
      wnoise_engine = MTPowerSpectrumEngine(
          n.npts(), wnoise_engine.time_bandwidth_product(),
          wnoise_engine.number_tapers());
    }
...
void CNR3CDecon::compute_gwl_inverse() {
  try {
    if (taper_data)
      wavelet_taper->apply(this->wavelet);
    if (this->wavelet.npts() != FFTDeconOperator::nfft) {
      throw MsPASSError("CNR3CDecon::compute_gwl_inverse():  wavelet size and "
                        "fft size t0 not match - this should not happen and "
                        "indicates a bug that needs to be fixed",
                        ErrorSeverity::Fatal);
    }
  ...
  try {
    ...
    if (bo.bandwidth() < (this->decon_bandwidth_cutoff)) {
      ...
      ss << "Killed because estimated bandwidth=" << bo.bandwidth()
         << " dB is below threshold of " << this->decon_bandwidth_cutoff
         << endl;
    ...
    for (k = 0; k < 3; ++k) {
      ...
      */
      ...
      // for(j=0;j<FFTDeconOperator::nfft;++j) cout << wvec[j]<<"
      // "<<rftmp[j].imag()<<endl; cout << "Function output uses time
      // shift="<<t0_shift<<endl;
...
TimeSeries CNR3CDecon::inverse_wavelet(double tshift0) {
  try {
    ...
    CoreTimeSeries invcore(this->FFTDeconOperator::FourierInverse(
        this->winv, *shapingwavelet.wavelet(), operator_dt, timeshift));
...
void CNR3CDecon::update_shaping_wavelet(const BandwidthData &bwd) {
  ...
  if (wtype == "butterworth") {
    ...
    throw MsPASSError(
        string("CNR3CDecon::update_shaping_wavelet:  ") +
...
PowerSpectrum CNR3CDecon::ThreeCPower(const Seismogram &d) {
  try {
    ...
    if (d.npts() != dnoise_engine.taper_length()) {
      dnoise_engine = MTPowerSpectrumEngine(
          d.npts(), dnoise_engine.time_bandwidth_product(),
          dnoise_engine.number_tapers());
    }
    for (int k = 0; k < 3; ++k) {
