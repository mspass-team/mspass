...
ArrivalTimeReference(Seismogram &tcsi, string arrival_time_key, TimeWindow tw) {
  double atime;
  ...
  if ((tw.start > tcso->t0()) || (tw.end < tcso->time(tcso->u.columns() - 1))) {
    ...
    if (jstart < 0)
      jstart = 0;
    if (jend >= tcso->u.columns())
      jend = tcso->u.columns() - 1;
...
ArrivalTimeReference(Ensemble<Seismogram> &tcei, string arrival_time_key,
                     TimeWindow tw) {
  ...
  shared_ptr<Ensemble<Seismogram>> tceo(
      new Ensemble<Seismogram>(dynamic_cast<Metadata &>(tcei), nmembers));
  ...
  for (indata = tcei.member.begin(); indata != tcei.member.end(); ++indata) {
    try {
      shared_ptr<Seismogram> tcs(
          ArrivalTimeReference(*indata, arrival_time_key, tw));
...
TimeSeries ExtractComponent(const Seismogram &tcs,
                            const unsigned int component) {
  /* No need to test for negative because we use an unsigned */
  if (component >= 3)
    throw MsPASSError(
        "ExtractComponent: illegal component number - must be 0, 1, or 2",
        ErrorSeverity::Invalid);
  ...
  try {
    ...
    TimeSeries result(dynamic_cast<const BasicTimeSeries &>(tcs),
                      ...
                      dynamic_cast<const ProcessingHistory &>(tcs), scomp);
...
Ensemble<TimeSeries> ExtractComponent(const Ensemble<Seismogram> &d,
  ...
  if (comp >= 3)
    throw MsPASSError("ExtractComponent(Ensmble): illegal component number - "
                      "must be 0,1, or 2",
                      ErrorSeverity::Invalid);
  try {
