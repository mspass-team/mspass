...
LinearTaper::LinearTaper(const double t0h, const double t1h, const double t1t,
                         const double t0t) {
  t0head = t0h;
  t1head = t1h;
  t0tail = t0t;
  ...
  if (!(head || tail)) {
    stringstream ss;
    ss << "LinearTaper constructor:  illegal input.  No valid taper parameters"
       << endl
...
CosineTaper::CosineTaper(const double t0h, const double t1h, const double t1t,
                         const double t0t) {
  t0head = t0h;
  t1head = t1h;
  t0tail = t0t;
  ...
  if (all) {
    if (d.npts() != taper.size()) {
      ...
      ss << "VectorTaper apply method:  size mismatch with data" << endl
         << "operator taper size=" << taper.size()
         << " but data vector length=" << d.npts() << endl
...
TopMute::TopMute(const double t0, const double t1, const std::string type) {
  ...
  if (t1 <= t0) {
    stringstream ss;
    ss << base_error << "Zero time (t0) must be less than end of mute time (t1)"
       << endl
  ...
  try {
    if (type == "linear") {
      taper =
          std::make_shared<LinearTaper>(t0, t1, t0 + 999999.0, t1 + 999999.0);
      taper->disable_tail();
    } else if (type == "cosine") {
      taper =
          std::make_shared<CosineTaper>(t0, t1, t0 + 999999.0, t1 + 999999.0);
      taper->disable_tail();
    } else {
      stringstream ss;
...
string TopMute::taper_type() const {
  ...
  if (dynamic_cast<const LinearTaper *>(rawptr))
    ...
    throw MsPASSError("TopMute::taper_type:  Internal taper dynamic cast does "
                      "not resolve;  this should not happen and is a bug",
                      ErrorSeverity::Fatal);
}
} // namespace mspass::algorithms
