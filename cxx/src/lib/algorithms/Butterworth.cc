...
Butterworth::Butterworth() {
  ...
  fstop = fnyq;
  astop = 0.01;
  this->bfdesign(fpass, apass, fstop, astop, &(this->npoles_hi),
                 &(this->f3db_hi));
  dt = 1.0;
  use_lo = false;
  use_hi = true;
...
Butterworth::Butterworth(const bool zp, const bool lcon, const bool hcon,
  ...
  try {
    ...
    if (ftype == "bandpass") {
      ...
      if (fdmeth == "corner_pole") {
        ...
        an error if the points do not define the right sense of pass versus
        cut*/
      ...
      if (fdmeth == "corner_pole") {
        ...
        if (f3db_lo < f3db_hi) {
          throw MsPASSError(base_error +
                                "Illegal corner frequencies for band reject "
                                "filter definition" +
                                "For a band reject low corner must be larger "
                                "than high corner (pass becomes cut to reject)",
        ...
        fstop = md.get<double>("fstop_low");
        astop = md.get<double>("astop_low");
        this->bfdesign(fpass * dt, apass, fstop * dt, astop, &(this->npoles_lo),
                       &(this->f3db_lo));
        fpass = md.get<double>("fpass_high");
        apass = md.get<double>("apass_high");
        fstop = md.get<double>("fstop_high");
        astop = md.get<double>("astop_high");
        this->bfdesign(fpass * dt, apass, fstop * dt, astop, &(this->npoles_hi),
                       &(this->f3db_hi));
      }
    } else {
...
void Butterworth::bfdesign(double fpass, double apass, double fstop,
                           double astop, int *npoles, double *f3db) {
  ...
  if (fstop > fpass) {
    fnpoles =
        log((1.0 / (apass * apass) - 1.0) / (1.0 / (astop * astop) - 1.0)) /
...
void Butterworth::bflowcut(int npoles, double f3db, int n, double p[],
                           double q[]) {
  int jpair, j;
...
void Butterworth::bfhighcut(int npoles, double f3db, int n, double p[],
                            double q[]) {
  int jpair, j;
  double r, scale, theta, a, b1, b2, pj, pjm1, pjm2, qjm1, qjm2;
...
void Butterworth::set_lo(const double fstop, const double fpass,
  ...
  if (fstop > fpass) {
    stringstream ss;
    ss << base_error << "Illegal input for frequency points f_stop=" << fstop
       << " f_pass=" << fpass << endl
  ...
  if ((astop < 0.0) || (apass < 0.0)) {
    ...
    ss << base_error << "Illegal amplitude values of astop=" << astop
       << " and apass=" << apass << endl
       << "Amplitudes must be nonnegative" << endl;
    throw MsPASSError(ss.str(), ErrorSeverity::Invalid);
  }
