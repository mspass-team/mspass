  ...
  */
  ...
  if ((is < 0) || (ie > parent.npts())) {
    ...
    mess << "WindowData(CoreSeismogram):  Window mismatch" << endl
         ...
         << endl
         << "Window end time=" << tw.end << " is sample number " << ie << endl
...
Seismogram WindowData(const Seismogram &parent, const TimeWindow &tw) {
  ...
  */
  ...
  if ((is < 0) || (ie >= parent.npts())) {
    ...
    dead_return.kill();
    dead_return.elog.log_error("WindowData", mess.str(),
                               ErrorSeverity::Invalid);
    /* In this case it is appropriate to clear the data array  - this does
     * that*/
    dead_return.set_npts(0);
    return dead_return;
  }
  /* MAINTENANCE ISSUE:  The original implementation of this code used a copy
  constructor here to initalize result.   We realized that was very inefficient
  when slicing down long input signals like data blocked in day files. We
  converted to this algorithm BUT be warned it is somewhat fragile as it depends
  on some side effects of the implementation of Seismogram that could break it
  if the following assumptions changes:
...
TimeSeries WindowData(const TimeSeries &parent, const TimeWindow &tw) {
  ...
  */
  ...
  when slicing down long input signals like data blocked in day files. We
  converted to this algorithm BUT be warned it is somewhat fragile as it depends
  on some side effects of the implementation of TimeSeries that could break it
  if the following assumptions changes:
  1.  The constructor we use calls the set_npts method
  2.  that constructor also syncs npts metadata
