...
int cautious_compare(const TimeSeries &x, const TimeSeries &y,
                     const string key) {
  if (x.is_defined(key)) {
    if (y.is_defined(key)) {
      string valx, valy;
...
Seismogram BundleSEEDGroup(const std::vector<TimeSeries> &d, const size_t i0,
                           const size_t iend) {
  ...
  try {
    ...
    for (size_t i = i0; i <= iend; ++i) {
      /* Skip any members marked dead but put a special entry in
       * chans_this_group*/
      if (d[i].dead()) {
    ...
    if ((stations.size() != 1) || (networks.size() != 1) ||
      ...
      d3c = dogtag(bundle);
      if (hvec.size() > 0)
        d3c.add_many_inputs(hvec);
      d3c.kill();
      stringstream ss;
      ss << "Irregular grouping:  inconsistent seed name codes in group"
         << endl;
      if (stations.size() > 1) {
      ...
      if (sta2set.size() > 1) {
        ss << "List of inconsistent channel codes (first 2 characters should "
              "be unique and they are not):  ";
      ...
      if (nkeys == 3) {
        for (auto kptr = keys.begin(); kptr != keys.end(); ++kptr) {
          ...
          if (xref.count(*kptr) == 1) {
            xptr = xref.find(*kptr);
            bundle.push_back(
                dynamic_cast<const CoreTimeSeries &>(d[xptr->second]));
            if (!(d[xptr->second].is_empty())) {
              hvec.push_back(
                  dynamic_cast<ProcessingHistory *>(&(d_const[xptr->second])));
            }
          } else {
            ...
            pair<multimap<string, size_t>::iterator,
            ...
            if (!(d[jjmin].is_empty())) {
              hvec.push_back(
                  dynamic_cast<ProcessingHistory *>(&d_const[jjmin]));
            }
          }
        }
      } else {
    ...
    if ((nkeys == 3) && (nlive >= 3)) {
      ...
      if (nkeys < 3) {
        ss << "Insufficient data to generate Seismogram object." << endl
           << "Number of channels received  for bundling=" << bundle.size()
           << endl
        ...
        ss << "Excess channel keys in group received" << endl
           << "Number of unique channel names=" << nkeys << " which should be 3"
           << endl
           << "Channel names received: ";
        for (auto chanptr = chans_this_group.begin();
             chanptr != chans_this_group.end(); ++chanptr)
          ss << (*chanptr) << " ";
        ss << endl;
      }
      d3c.elog.log_error("BundleSEEDGroup", ss.str(), ErrorSeverity::Invalid);
...
/* channel and loc codes are relics when converting scalar data to Seismogram
   objects.  We call this function to remove them from outputs if they are
   defined.
*/
void clear_channel_metadata(Seismogram &d) {
  /* Do nothing to any datum marked dead */
  if (d.live()) {
    if (d.is_defined(SEISMICMD_chan))
      d.erase(SEISMICMD_chan);
    if (d.is_defined(SEISMICMD_loc))
      d.erase(SEISMICMD_loc);
  }
}
...
LoggingEnsemble<Seismogram> bundle_seed_data(LoggingEnsemble<TimeSeries> &d) {
  ...
  try {
    if (d.dead()) {
      ...
      d3c.elog += d.elog;
      stringstream ss;
      ss << "Received an ensemble marked dead; returning an empty ensemble "
            "cloning only ensemble Metadata and elog containers"
         << endl;
    ...
    LoggingEnsemble<Seismogram> ens3c(dynamic_cast<Metadata &>(d),
                                      d.member.size() / 3);
    ...
    for (i = 0, dptr = d.member.begin(); dptr != d.member.end(); ++i, ++dptr) {
      if (dptr->dead()) {
        ...
        if (dptr->is_defined(SEISMICMD_net) &&
          ...
          stringstream ss;
          ss << "Member " << i
             << " of input ensemble was marked dead and lacked required net, "
                "sta, chan, and loc keys"
             << endl;
          ss << "That datum was dropped from processing and could not be "
                "recovered.  History will be incomplete"
             << endl;
          ...
          if (nlive >= 3) {
            ...
            if ((iend - i0) == 2) {
              ...
              for (size_t k = 0; k < 3; ++k) {
                work.push_back(
                    dynamic_cast<CoreTimeSeries &>(d.member[i0 + k]));
                if (!(d.member[i0 + k].is_empty())) {
                  hvec.push_back(
                      dynamic_cast<ProcessingHistory *>(&d.member[i0 + k]));
                }
              }
              Seismogram d3c;
              try {
            ...
            stringstream ss;
            ss << "Insufficient data to generate Seismogram object." << endl
               << "Number of channels received  for bundling=" << work.size()
               << endl
               << "Number marked live=" << nlive << endl
               << "Number live must be at least 3" << endl;
            d3c.elog.log_error("bundle_seed_data", ss.str(),
