...
/*! \brief Colored-noise regularized three-component deconvolution engine.

This operator builds frequency-domain inverse filters using either generalized
water-level or colored-noise damping regularization and applies them to
...
public:
  ...
  */
  ...
  /*! Initialize the inverse operator from signal and noise time series. */
  void
  ...
  /*! Apply the deconvolution operator to a three-component datum. */
  ...
  Overloaded version returns average power spectrum of all three components.
  Measure of overall 3c power comparable to scalar signal power.   Could do sum
  of component spectra but that would cause a scaling problem for use in
  regularization. For most data this will tend to be dominated by the component
  ...
  */
  ...
  /*! Return the actual output generated by applying inverse and wavelet. */
  ...
  /*! Return the inverse wavelet for a supplied input wavelet. */
  mspass::seismic::TimeSeries
  inverse_wavelet(const mspass::seismic::TimeSeries &wavelet,
                  const double t0shift);
  /*! Return quality-control metrics from the most recent processing. */
