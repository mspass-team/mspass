...
public:
  ...
  CNR3CDecon();
  /*! \brief Construct from a tree of parameters loaded into an AntelopePf
  object.
  ...
  */
   ...
   *
   ...
   \param loadnoise when true the function will attempt to load data for
     the noise based regularization from the noise window defined for the
   operator.
   \exception MsPASSError may be thrown for a number of potential error
   conditions.
   */
  ...
  /*! \brief Load noise data directly.
   ...
   slightly larger than the operator length to be consistent with the
   expectations of the multitaper method.   If the noise window is short the
   spectrum is computed but will be biased to lower amplitudes because of zero
   padding. That happens because the operator will not recompute Slepian tapers
   if the data are short.
   */
  void loadnoise_data(const mspass::seismic::Seismogram &n);
  /*! \brief Load noise estimate directly as a PowerSpectrum object.
   *
   The actual noise regularization is computed by this algorithm from an
   internally stored PowerSpectrum object.   This method allows the power
   spectrum to be computed by some other method or using previously computed
   estimates rather than computing it through loadnoise.  For instance, one
   might want to use a robust form of Welch's method to estimate preevent noise
   that would be immune to spikes and sporatic noise bursts.

   */
  void loadnoise_data(const mspass::seismic::PowerSpectrum &n);
  /*! \brief Load data defining the wavelet to use for deconvolution.

    This algorithm assumes a deterministic model for deconvolution.  That is, we
    have an estimate of the source wavelet.   In conventional receiver functions
    this is the vertical or longitudinal component.   In current array methods
    it is always some stack (not necessarily a simple average) of vertical or
    longitudinal data from an ensemble.

    It is VERY IMPORTANT to realize that loadwavelet initiates the calculation
    of the inverse for the deconvolution.   That allows this same processing
    object to be efficiently used in array deconvolution and single station
    deconvolution.   For single station estimates loadwavelet should be called
    on each seismogram.  For array methods loadwavelet should be called once for
    the ensemble (common source gather) to which a wavelet is linked. The
    inverse is then applied to very signal in the ensemble with process.

    \param w is the wavelet.  Must be in relative time with 0 set to the
    estimated first break time.
    */
  ...
  /*! \brief Load noise estimate for wavelet signal directly as a PowerSpectrum
   object.
   *
   The actual noise regularization is computed by this algorithm from an
   internally stored PowerSpectrum object.   This method allows the power
   spectrum to be computed by some other method or using previously computed
   estimates rather than computing it through loadnoise.
