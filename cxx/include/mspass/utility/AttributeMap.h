...
namespace mspass {
namespace utility {
/*! \brief  Defines properties of a database attribute and link to internal
names.
*
...
public:
   ...
   *  This constructor exists to avoid automatic construction with no
   * parameters, but it should not be used. */
  AttributeProperties();
  /*! \brief Create using a string description.
   * The standard construction of an associative array uses this constructor
   * driven by a series of input lines.  The input line that is parsed must
   * contain these member attributes in the following order:
   * -# internal_name
   * -# db_attribute_name
   * -# db_table_name
   * -# mdt
   *
   * The mdt field must be one of "REAL", "real", "STRING", "string", "INT",
   * "int", "BOOL", "BOOLEAN", "bool", or "boolean". Any other field with cause
   * this constructor to throw an exception.
  ...
  Database attribute names can and often do occur in multiple tables.  This
  method returns a full name that is used to uniquely define that attribute in a
  particular table.  Although this could be generalized, for the present this is
  always returned in the form used by Antelope/datascope:  that is the string is
 ...
 *
 * This object is used to link a set of internally defined parameters tagged
 * with a name to an external name convention.  The working model for external
 * names is attribute names defined in a relational database schema, but the
 * concept involved is more general.  That is, the intent of this interface is a
 * general way to between one set of parameter names and another.  This could be
 * used, for example, to map between header variable names in SEGY or SAC and
 * some internal name convention.  The relation of the map defined by this
 * object is implicitly assumed to be one-to-one because of the use of the STL
 * map to define the relationship.  Because the map is keyed by the internal
 * name lookup is also intended only for finding the external names associated
 * with a particular internal parameter.  The primary use of this object in the
 * MsPASS library is to define a global mapping operator for a particular
 * database schema.  That is, the most common construct is to build this object
...
public:
   ...
   * The key thing a user must know is the proper way to retrieve an element
   * from a map and handle the possibility that the requested item is not known
   * to the map.  Consult the web or the source code for libMsPASS if you don't
   * know how to do this.  */
  ...
  AttributeMap();
  /*! \brief Create mapping for a specified namespace tag (usually a schema
   * name).
   *
   ...
   * header fields and an internal name convention.  The interface simply
   * assumes this keyword can be used to establish a mechanism for creating this
   * beast through an unspecified mechanism.  i.e. the interface is blind to the
  ...
  */
  ...
  aliases(const std::string key) const;
  /*! Overload for literals. */
  std::map<std::string, AttributeProperties> aliases(const char *key) const;
  /*! Returns an ordered list of table names to try in extracting an alias
  named.

  Aliases present an issue on input.  Because many attribute names appear in
  multiple tables (an essential thing, in fact, for a relational database to
  work) input of an attribute that is a generic label for such an attribute can
  be problematic.  This method returns an ordered list of tables that provide
