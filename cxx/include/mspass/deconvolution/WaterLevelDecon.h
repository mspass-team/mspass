...
public:
    ...
    WaterLevelDecon(const mspass::Metadata &md);
    WaterLevelDecon(const mspass::Metadata &md,const std::vector<double> &wavelet,const std::vector<double> &data);
...
private:
    int read_metadata(const mspass::Metadata &md);
    int apply();
    double wlv;
    /* QC metrics.   */
