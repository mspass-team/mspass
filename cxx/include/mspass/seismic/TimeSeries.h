...
public:
  ...
  TimeSeries(const size_t nsamples)
      : mspass::seismic::CoreTimeSeries(nsamples),
        mspass::utility::ProcessingHistory() {};
  /*! Partially construct from components.

      There are times one wants to use the Metadata area as a template to
      flesh out a CoreTimeSeries as what might be called skin and bones:  skin
     is Metadata and bones as BasicTimeSeries data.   This constructor
     initializes those two base classes but does not fully a valid data vector.
     It only attempts to fetch the number of points expected for the data vector
     using the npts metadata (integer) key (i.e. it sets npts to
     md.get_int("npts")). It then creates the data vector of that length and
     initialzies it to all zeros.
  ...
  */
  ...
  TimeSeries(const mspass::seismic::CoreTimeSeries &d)
      : mspass::seismic::CoreTimeSeries(d),
        mspass::utility::ProcessingHistory() {};
  ...
  */
  ...
  function */
  TimeSeries(const mspass::seismic::BasicTimeSeries &b,
