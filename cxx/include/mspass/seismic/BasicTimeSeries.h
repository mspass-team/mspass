...
enum class TimeReferenceType {
  UTC,     /*!< Use an absolute (usually UTC) time base - previously absolute in
              SEISPP. */
...
public:
  ...
  **/
  int sample_number(double t) const { return (round((t - mt0) / mdt)); };
  /*!
  ...
  **/
  ...
  bool shifted() const { return t0shift_is_valid; };
  /*! Return the current value of the t0shift attribute - argument to restore to
   * UTC */
  double get_t0shift() const { return this->t0shift; };
  ...
  \param tshift - time shift applied to data before switching data to relative
  time mode.
  **/
  ...
  /*! Relative to absolute time conversion.
   ...
   This method returns data previously converted to relative back to UTC using
   the internally stored time shift attribute. */
  virtual void rtoa();
  /*! Shift the reference time.

    Sometimes we need to shift the reference time t0.  An example is a moveout
    correction. This method shifts the reference time by dt.   Note a positive
    dt means data aligned to zero will be shifted left because relative time is
    t-t0.
    */
  virtual void shift(const double dt);
  /*! Returns true of data are marked valid (live).  */
  bool live() const { return this->mlive; };
  /*! Return true if the data have been marked bad (killed) - inverse of
   * live()*/
  ...
  /*! Return time of first data sample.  An epoch time or relative time
  depending on TimeReferenceType private variable tref*/
  ...
  */
  virtual void set_dt(const double sample_interval) { mdt = sample_interval; };
  ...
  because children may want to do more than just set the attribute in this
  base class.  In MsPASS that means keeping the Metadata attributes that
  define the number of points in sync with the data.   Aliases further
  complicate that issue so it is not trivial.   Other data objects that could be
  derived form this base could have similar issues.

  \param npts is the new number of points to set.
  */
  virtual void set_npts(const size_t npts) { nsamp = npts; };
  ...
  */
  virtual void set_t0(const double t0in) { mt0 = t0in; };
...
Time series data may have multiple concepts of what the time standard is.
  ...
  */
  void set_tref(const TimeReferenceType newtref) { tref = newtref; };
...
protected:
  ...
  **/
  ...
  to any external standard but are relative to some arbitrary reference that
  must ascertained by the algorithm by some other means (in seispp this is
  normally done through a metadata object).  A classic example is multichannel
